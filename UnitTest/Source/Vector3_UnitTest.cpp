...
TEST_CASE("Vector3", "[.all][vector][Vector3]")
{
	SECTION("Instantiation")
	{
		...
		std::cout << allParam << "\t" << allParamGlm.x << " " << allParamGlm.y << "  " << allParamGlm.z << std::endl;
		CHECK(memcmp(&allParam, &allParamGlm, sizeof LibMath::Vector3));
	}
	...
	SECTION("Comparator")
	{
		...
		CHECK(LibMath::Vector3{ 2.5f, .5f, 2.f } != LibMath::Vector3{ 2.5f, .5f, 2.1f });
	...
	SECTION("Conversion")
	{
		LibMath::Vector3 base3{ 1.f, 2.f, 1.f };
		const LibMath::Vector2 base2{ 1.f, 2.f };
		const LibMath::Vector4 base4{ 1.f, 2.f, 1.f, 1.f };

		{
			LibMath::Vector2 checkBase3 = base3;
			CHECK(checkBase3 == base2);

			LibMath::Vector4 checkBase4 = base3;
